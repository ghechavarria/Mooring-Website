...
- **Footer:** Closing tagline strip pinned to the **bottom of the hero viewport** (flex chain: hero `min-h-[calc(100svh-4.25rem)]`, children block `flex-1 justify-center`, tagline `shrink-0`). Styled **`bg-erp`** (`#0075FF`) with **`text-white/90`** and **`border-erp-600`** — benefit-forward copy, e.g. **“Mooric ERP — built for LOs who are tired of running their pipeline out of Excel.”** The pulse above remains **decorative** (`aria-hidden`); this line nudges interest without claiming live tenant data.
