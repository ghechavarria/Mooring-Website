...
- **Status lines:** Mortgage-operations teaser copy rotates on the **same timer** as the pulse BPM. **Integrated:** **`line-clamp-2`** on **`< sm`** (tight band; slot **`h-auto`**), **`line-clamp-4`** from **`sm`** up inside a fixed-height slot so the **tablet/desktop** AI row stays stable when copy changes.
