...
- **Global header:** Sticky top bar (`Header.tsx`) — **opaque `bg-white`**, full width (`w-full min-w-0`), soft **double shadow** so it stays readable over **dark hero / slate strips** without translucent blur. **Brand:** **`mooring-small-logo.png`** plus **Mooring ERP** wordmark (**Mooring** in organ, **ERP** in **`#1e70d6`**) beside it, nav **`font-medium text-organ-800`**, **`.btn-primary-silver`** for briefing. The inner row uses **`.layout-header`** (fluid full width, no `max-w` cap — see [layout-shell.md](./layout-shell.md)) with **`justify-between`** and **`xl:gap-x-6`**: brand stays **left**, **nav + briefing** stay **right** from **`xl`** up with **`xl:ml-10` / `2xl:ml-14` / `min-[1800px]:ml-20`** on the link cluster (below **`xl`**, the **drawer** pattern is used — see [mobile-nav-and-layout.md](./mobile-nav-and-layout.md)). The integrated hero uses **`.layout-header-px`** on full-bleed rows so **session chrome** lines up with this rail.
