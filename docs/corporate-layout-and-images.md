...
- **Navigation and UI:** Inter; header brand lockup is **Mooric ERP** (`square-logo-blue.svg` + styled text in `Header.tsx`).
...
| Hero (motion band) | `HeroMotionBand.tsx` (in **`App.tsx`** after `Workflow`) | Full strip: `HeroLattice` grid, noise, blobs, scan line, **Mooric ERP** wordmark, mortgage caption; **ink–navy** base with **sky / ERP blue** and **accent** radials. Honors `prefers-reduced-motion`. |
