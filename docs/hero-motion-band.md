...
Full-bleed **dark organic slate** strip with **gold** lattice, motion, and the **BaseReality** wordmark. **`App.tsx`** renders it **after** **`Workflow`** and **before** **`StatsSection`**.
...
- **Word:** **BaseReality** — `Montserrat`, gold → ivory gradient fill, `bg-clip-text`, `animate-shimmer-slow`, uppercase tracking.
...
`useReducedMotion()` disables drifting blobs, the moving scan line, and the pulsing glow; the lattice grid and gradient **BaseReality** word remain readable.
