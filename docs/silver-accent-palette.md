...
**`tide`** is set to the same hex as **`accent`** for legacy naming. Primary CTAs use **`.btn-primary-silver`** with the **`erp`** brand blue (**`#1e70d6`**, same as the nav **ERP** wordmark; see [`tailwind.config.js`](../tailwind.config.js) and [button-primary-silver.md](./button-primary-silver.md)). Broader system notes: [theme-mooric-dashboard.md](./theme-mooric-dashboard.md).
